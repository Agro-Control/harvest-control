...
const getFieldClimateAnualDetail = async (talhao_id: number, data_inicio : string, data_fim : string ) => {
...
export const useGetFieldClimateAnualDetail = (enabled: boolean, talhao_id: number, data_inicio : string, data_fim : string ) => {
    return useQuery({
        queryKey: ["field_climate_anual_detail", talhao_id],
        enabled: !!enabled, 
        queryFn: () => getFieldClimateAnualDetail(talhao_id, data_inicio, data_fim),
    });
};