...
const getOperativeMachinesByGroup = async (grupo_id: number | null) => {
...
export const useGetOperativeMachinesByGroup = (grupo_id: number | null) => {
    return useQuery({
        queryKey: ["machines_by_group", grupo_id],
        enabled: true, 
        queryFn: () => getOperativeMachinesByGroup(grupo_id),
    });
};